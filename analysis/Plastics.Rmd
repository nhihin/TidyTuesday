...
clean_plastics <- plastics %>%
  ...
  dplyr::filter(!parent_company %in% c("#Error!", "Unbranded", "null", "NULL","Null", "Grand Total"),
                !country %in% c("EMPTY"))
```
...
```{r}
clean_plastics3 <- clean_plastics2[, c(4:11)]
clean_plastics3[is.na(clean_plastics3)] <- 0

clean_plastics2[, c(4:11)] <- clean_plastics3
```
...
```{r}
clean_plastics <- plastics %>%
  dplyr::filter(!is.na(parent_company),
